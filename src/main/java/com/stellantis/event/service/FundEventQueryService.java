...
public class FundEventQueryService {
	...
	public FundEventDetailDto getEventDetails(String fundCode, UUID eventId) {
		...
		FundEventEntity event = fundEventRepository.findByIdAndFundCode(eventId, fundCode)
				.orElseThrow(() -> new EventNotFoundException(eventId.toString()));

		List<EventFileLogEntity> reportFiles = eventFileLogRepository
				.findByFundEventEntity_IdAndCategoryOrderByCreatedAtAsc(eventId, "REPORT");

		List<EventFileLogEntity> outputFiles = eventFileLogRepository
				.findByFundEventEntity_IdAndCategoryOrderByCreatedAtAsc(eventId, "OUTPUT_FILE");

		return FundEventDetailDto.builder().eventId(event.getId())
				.eventType(event.getEventType() != null ? event.getEventType().name() : null)
				.status(event.getStatus().name()).actions(computeActions(event.getStatus()))
				.reports(reportFiles.stream().map(this::toFileEntry).collect(Collectors.toList()))
				.outputFiles(outputFiles.stream().map(this::toFileEntry).collect(Collectors.toList())).build();
	}

	private FileEntryDto toFileEntry(EventFileLogEntity efl) {
		return FileEntryDto.builder().fileId(efl.getId()).fileName(efl.getFileName()).fileType(efl.getFileType())
				.sizeBytes(efl.getSizeBytes()).isTiersRestricted(efl.getTiersRestricted()).build();
	}
